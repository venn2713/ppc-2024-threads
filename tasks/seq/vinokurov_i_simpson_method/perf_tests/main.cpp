...
TEST(vinokurov_i_simpson_method_seq, test_pipeline_run) {
  const int a = 0;
  const int b = 1;
  const int c = 0;
  const int d = 1;
  ...
  // Create Task
  auto testTaskSequential = std::make_shared<SimpsonMethodSequential>(taskDataSeq, fn);
...
TEST(vinokurov_i_simpson_method_seq, test_task_run) {
  const int a = 0;
  const int b = 1;
  const int c = 0;
  const int d = 1;
