...
TEST(ivanov_n_int_simpson_seq, test_pipeline_run) {
  const int a = 0;
  const int b = 2;
  const int c = 0;
  const int d = 2;
  ...
  // Create Task
  auto testTaskSequential = std::make_shared<TestTaskSequentialSimpson>(taskDataSeq, function);
...
TEST(ivanov_n_int_simpson_seq, test_task_run) {
  const int a = 0;
  const int b = 2;
  const int c = 0;
  const int d = 2;
