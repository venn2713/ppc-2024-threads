...
inline void apply_filter(const size_t& w, const size_t& h, const uint32_t* img, const float* kernel,
                         uint32_t* img_res) {
  kochetov_n_gauss::apply_filter_line<true>(w, h, img, kernel, img_res, 0);
  for (size_t x = 1; x < w - 1; x++) {
    kochetov_n_gauss::apply_filter_line<false>(w, h, img, kernel, img_res, x);
  }
  if (w != 1) {
    kochetov_n_gauss::apply_filter_line<true>(w, h, img, kernel, img_res, w - 1);
  }
}
