...
SSobelSeq::GrayScale SSobelSeq::getPixel(const std::vector<SSobelSeq::GrayScale>& image,
...
bool SSobelSeq::pre_processing() {
    try {
        ...
        for (size_t i = 0; i < imgSize; ++i) {
            colored_img.push_back(RGB{rawData[i * 3],
                                      rawData[i * 3 + 1],
                                      rawData[i * 3 + 2]});
        }
...
bool SSobelSeq::post_processing() {
    try {
        ...
        for (size_t i = 0; i < grayscale_img.size(); ++i) {
            SSobelSeq::GrayScale *pixel = reinterpret_cast<SSobelSeq::GrayScale*>(taskData->outputs[0]
                                                                + i);
            *pixel = result[i];
        }
    } catch (...) {
