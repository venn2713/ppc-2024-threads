...
bool LinearFiltering::validation() {
    internal_order_test();
    return taskData->inputs_count[0] >= 3 &&
...
void LinearFiltering::imageSet(std::vector<uint8_t> _image) {
    image = _image;
}
...
void LinearFiltering::applyGaussianFilter() {
    if (width < blockWidth || height < blockHeight)
        throw "Error: Image size is less than block size!";
        ...
        for (int j = 0; j < width; j += blockWidth)
            for (int k = i; k < i + blockHeight; k++)
                for (int l = j; l < j + blockWidth; l++) {
                    ...
                    resultImage[k * width + l] =
                        sum / std::accumulate(gaussKernel.begin(),
                                              gaussKernel.end(), 0);
                }
