...
bool KhodyrevTaskSequential::run() {
  ...
  try {
    ...
    sort(pixels.begin(), pixels.end(), [&](const Pixel& p1, const Pixel& p2)
     -> bool {
        int orientation = (p1.x - start_point.x) * (p2.y - start_point.y) -
                          (p2.x - start_point.x) * (p1.y - start_point.y);
        if (orientation == 0)
    ...
    for (size_t i = 2; i < pixels.size(); ++i) {
        while (hull.size() >= 2) {
            ...
            hull.pop();
            Pixel p2 = hull.top();
            if ((p2.x - p1.x) * (pixels[i].y - p1.y) - (p2.y - p1.y) *
             (pixels[i].x - p1.x) < 0) {
                hull.push(p1);
                break;
            }
