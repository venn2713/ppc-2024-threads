...
TEST(kistrimova_e_graham_alg_seq, test_pipeline_run) {
  ...
  // Create Task
  auto sequentialTask = std::make_shared<KistrimovaETaskSeq::GrahamAlgTask>(data);
...
TEST(kistrimova_e_graham_alg_seq, test_task_run) {
  const int count = 5000;
  // Create data
  std::vector<double> pX;
  std::vector<double> pY;
