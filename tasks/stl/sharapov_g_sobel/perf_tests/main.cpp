...
TEST(sharapov_g_sobel_stl, test_pipeline_run) {
  int sideLen = SIZE_IMG;
  std::vector<SSobelStl::RGB> ColoredImage = SSobelStl::generateColorImage(sideLen, sideLen, 1984);
  std::vector<SSobelStl::GrayScale> EdgeImage(ColoredImage.size());
  ...
  // Task
  auto SobelSeq = std::make_shared<SSobelStl>(taskDataSeq);
...
TEST(sharapov_g_sobel_stl, test_task_run) {
  int sideLen = SIZE_IMG;
  std::vector<SSobelStl::RGB> ColoredImage = SSobelStl::generateColorImage(sideLen, sideLen, 1984);
  std::vector<SSobelStl::GrayScale> EdgeImage(ColoredImage.size());
